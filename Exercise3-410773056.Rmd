...
```{r ans12}
#which(classData$`公-10-06` == "有看公告" )
subClassData <- classData[classData$`公-10-06` == 1,]
```
...
```{r ans14}
e <- (classData$`練2018-10-17-1` == 1)
f <- (classData$`公-10-06` == "沒看公告")
g <- e[f]
as.factor(as.numeric(g)) -> g
levels(g) <- c("沒看公告沒記錄","沒看公告有記錄")
table(g)
9/25 -> ratio1
ratio1
```
...
```{r ans23}
(deadline - hwData$Timestamp) -> hwData$early
...
round(as.numeric(hwData$early)) -> hwData$early
