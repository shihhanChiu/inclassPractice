...
title: "Exercise 7"
params:
  id: 410773056（留意要與studentID:間空一格）
  name: 邱詩涵（留意要與name:間空一格）
  repoURL: https://github.com/shihhanChiu/inclassPractice（選填）
date: "01/08/2019"
---
...
```{r ans14}
transcriptData$學年學期 <- str_c(transcriptData$學年,transcriptData$學期)
subsample <- transcriptData[,c(10,11)]
subsample <- unique(subsample)
subsample %>%
  group_by(學號)%>%
  mutate(
    學年學期 = n()
...
studentID <- subsample[(subsample$學年學期 <= 8),][,1]
